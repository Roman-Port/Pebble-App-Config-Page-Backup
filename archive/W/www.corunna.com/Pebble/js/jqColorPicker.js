;(function($, Colors, undefined){
	...
	var $document = $(document),
		...
		_css = '.cp-color-picker{position:absolute;overflow:hidden;z-index:999999;padding:1p' +
			'x 1px 0;background-color:#444;color:#bbb;font-family:Arial,Helve' +
			...
			'bottom:1px;background:linear-gradient(to right,#FFF,rgba(255,255' +
			...
			'-left:1px;height:128px;width:20px;background:linear-gradient(red' +
			...
			'p-alpha{clear:both;width:100%;height:11px;margin:1px 0;backgroun' +
	...
	function toggle(event) {
		...
		if (event) {
			...
			(_$UI || build()).css({
				// 'width': _$UI[0]._width,
				'left': (_$UI[0]._left = position.left) + 100 -
					((_$UI[0]._left = _$UI[0]._left + _$UI[0]._width -
					($window.scrollLeft() + $window.width())) + gap > 0 ?
					_$UI[0]._left + gap : 0),
				'top': (_$UI[0]._top = position.top - 140 + $this.outerHeight()) -
		...
		var noop = function(){};
 		...
 		options = $.extend({
			...
			renderCallback: function($elm,toggle) {
                          ...
                          $('.myface').css('background-color','#'+colors.HEX);
                        },
			buildCallback: noop,
			body: document.body,
			scrollResize: true,
