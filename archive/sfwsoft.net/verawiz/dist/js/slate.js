...
(function($, Sortable) {
  ...
  $.extend($.fn, {
    ...
    itemDynamicListLookup: function() {
      this.each(function() {
        var $list = $(this);
        $list.children('label').each(function() {
		  console.log($(this));
		...
		var del_items = document.getElementsByClassName("delete-item");
		for (var i = 0; i < del_items.length; i++) {
			del_items[i].addEventListener('click', function() {
				$(this).parent().remove();
			}, false);
		}
        ...
        $addButton.click(function() {
		  var options_arr = $list.attr('data-list').split(",");
		  var length = options_arr.length;
		  var options_html = "";
		  for (var i = 0; i < length; i++) {
		    options_html += '<option class="item-select-option" dir="ltr">' + options_arr[i] + '</option>';
		  }
          ...
          var $inbox = $('<div class="item">'
						...
						+ '=>'
                          //+ '<input class="item-input" type="text" name="focus-box">'
							 + '<select id="theme" dir="rtl" name="theme" class="item-select">'
							 + options_html
							 + '</select>'
						+ '</label>'
  ...
  $(function() {
	...
	$('.item-dynamic-listlookup').itemDynamicListLookup();
  });
}(Zepto, Sortable));
